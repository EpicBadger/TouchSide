...
namespace touchsides
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private void getAllWords(string fileContent)
        {
            ...
            var WordDetails = wordStats.Select(f => $"{f.Word} - Occurrences:{f.Occurrences}, Score: {f.Score}").ToArray();
            ...
            mostFrequentTB.Text = $"{ String.Join(", ", this.wordStats.Where(wordDetail => wordDetail.Occurrences == MaxOccurrences).Select(f => f.Word.Trim())) } \nWith {MaxOccurrences} occurrence(s)";

            mostFrequent7CharTB.Text = $"{ String.Join(", ", this.wordStats.Where(wd => wd.Word.Length == 7 && wd.Occurrences == Max7CharOccurrences).Select(wd => wd.Word.Trim())) } \nWith {Max7CharOccurrences} occurrence(s)";
